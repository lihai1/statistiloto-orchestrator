...
- **FR-5** Save and manage generated/favorite number sets per user account
  (CRUD: create, list, delete). Duplicate detection rejects internal dups and
  exact-set dups within the same category. Sets are shareable (per-set share
  link from the Number Wallet).
...
- **FR-8** Admin operations: configure LLM at runtime (stored configs CRUD +
  ...
  view token usage, reindex RAG. Agent admin tools (gated by the small-model
  admin-ops guard, keyword-mapped in `app/rag/admin_commands.yaml`): web search
  (`search_web`), DB table listing (`list_db_tables`), and ad-hoc SQL queries
  (`query_db`) for admin diagnostics.
...
- Multi-request detection: splits on EN/HE conjunctions and asks the user to
  pick one operation when >1 is detected (generate-then-save exempted).
...
- Admin-ops guard: small-model keyword→tool mappings
  (`app/rag/admin_commands.yaml`) for `search_web`, `list_db_tables`,
  `query_db`, `read_token_usage`, `query_audit_log`, `trigger_scraper`.

- Redis publisher for `/chat/stream` progress events
  (`app/redis_client.py`); inline SSE fallback when Redis is unavailable.
...
- Shared by the Java BFF (subscriber, via Lettuce) and the Python agent
  (publisher, via `redis.asyncio`). Both gate startup on `redis: service_healthy`
  and degrade to inline SSE when unavailable.

- Prod override adds `restart: always` and resource limits (1 CPU / 512M).
- Port 6379 (internal).
